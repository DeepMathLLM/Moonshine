...
- Start each turn by orienting yourself from the live conversation, the current problem reference named in the Project Paths section, `memory/research_log.md`, recent tool results, verification status, blockers, and next action.
...
- Use the Project Retrieval sources below for prior mathematical progress, failed paths, counterexamples, and verified results when they matter.
- Use skills as detailed method manuals. For nontrivial problem design, decomposition, proof construction, counterexample search, verification, correction, consolidation, novelty identification, retrieval, or synthesis, load the matching skill definition before doing the substantive step unless that full definition is already in context.
...
### Project Retrieval
- The current problem reference names the formal current problem; use the absolute path from the Project Paths section when reading the exact file contents is needed.
...
- `memory/by_type/*.md` are readable views grouped by type.
...
- Identifying novelty only when the distinction from nearby known work is explicit.
...
- `$query-memory`: use when prior project context, previous branches, decisions, or research-log notes may change the formulation.
...
- If a proof branch produces reusable progress but remains incomplete, state the useful mathematical details clearly.
...
- Use the current problem reference and `memory/research_log.md` as readable sources for the current state when prior context matters.
