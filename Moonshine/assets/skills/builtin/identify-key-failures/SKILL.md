...
description: Record failed mathematical attempts, identify the key failure points, and extract lessons that should guide the next research plan.
...
3. For each failed attempt, record the core idea in one or two sentences together with the main steps, constructions, or key lemmas used.
4. Record the point of failure precisely: where the argument got stuck, where a condition was misused, where a calculation broke, or why a proposed counterexample or construction failed.
...
- `search_knowledge`: Check whether stored conclusions, theorem summaries, or known obstructions explain the failure.
...
- The archival pass will save clear failed-path analysis into `research_log.jsonl` for later retrieval.
