...
description: Check for logical errors and flaws in the solution process of a subproblem.
...
## Execution Steps
1. Restate the target subproblem, the assumptions, and the conclusion the solution claims to establish.
2. Use `query_memory` or `search_knowledge` when prior verifier reports, cited lemmas, or saved branch context are needed.
...
- `query_memory`: Retrieve prior logic failures, branch notes, or saved reasoning context.
- `search_knowledge`: Retrieve stored lemmas, theorem statements, or definitions needed to judge the logical chain.
