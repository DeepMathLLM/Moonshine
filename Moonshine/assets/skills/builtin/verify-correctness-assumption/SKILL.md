...
description: Check whether every condition and assumption involved in a subproblem has actually been used in the solution process.
...
## Execution Steps
1. Restate the target subproblem, its known conditions, and its target conclusion.
2. Use `query_memory` or `search_knowledge` when prior verifier reports, saved assumptions, or related lemmas are needed.
...
- `query_memory`: Retrieve saved assumptions, failed verifier reports, or related branch context.
- `search_knowledge`: Retrieve stored lemmas or definitions when the assumption set depends on prior knowledge.
