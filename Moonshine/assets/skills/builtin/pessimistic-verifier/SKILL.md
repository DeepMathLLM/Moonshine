...
description: Legacy fallback verifier that runs independent pessimistic proof audits and rejects the proof if any reviewer finds a wrong or inconclusive result.
...
## Execution Steps
1. Restate the target statement and all assumptions.
2. Use `query_memory` or `search_knowledge` if prior verifier reports, stored lemmas, definitions, or assumptions are needed.
...
- `search_knowledge`: Check cited stored conclusions and assumptions.
