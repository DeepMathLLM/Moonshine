...
## Ordinary Work
### Primary Skills
- `$query-memory`: retrieve prior conversation state, project history, or saved preferences when they may matter.
...
- `$record-novelty`: preserve a genuinely new concept, method, or theory-level idea when one appears.

### Primary Tools
- `query_memory`: retrieve prior session, project, or research-log context.
- `search_knowledge`: recover stored stable conclusions before re-deriving them.
...
### Formal State Updates
- State the selected or revised problem clearly.
- State important mathematical progress clearly; the archival pass will save it into `research_log.jsonl` for later retrieval.
- When the saved evidence supports a formal move into solving, proceed into problem-solving work directly in the conversation.
...
- State important conclusions, failed paths, counterexamples, and proof progress clearly; the archival pass will save them into `research_log.jsonl`.
