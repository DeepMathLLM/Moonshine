...
SKILL_ALIAS_MAP = {
    ...
    "extract-project-memory": {"project-context", "project-decisions", "project-progress"},
...
class HeuristicMemoryExtractor(object):
    ...
    def extract(self, user_message: str, assistant_message: str, project_slug: Optional[str]) -> ExtractedItems:
        ...
        if project_slug and any(marker in combined_text.lower() for marker in ["decide", "decision", "next step", "plan", "goal"]):
            result.entries.append(
                self._entry(
