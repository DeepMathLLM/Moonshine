...
class ContextManager(object):
    ...
    def _render_message_line(self, item: Dict[str, object]) -> str:
        ...
        content = self._stringify_payload(item.get("content", "")).strip()
        fragments = []
        if content:
            fragments.append("[%s] %s" % (role, content))
        ...
        if tool_calls:
            ...
            for call in tool_calls:
                ...
                if not name:
                    name = str(call.get("id", "")).strip() or "tool-call"
                arguments = self._stringify_payload(function.get("arguments", ""))
                if arguments:
                    tool_names.append("%s(%s)" % (name, shorten(arguments, 160)))
                else:
                    tool_names.append(name)
            fragments.append("[assistant-tool-calls] %s" % "; ".join(tool_names))
        if not fragments:
            fragments.append("[%s]" % role)
        return "\n".join(fragments)
    ...
    def _summarize_with_provider(self, *, purpose: str, text: str, token_budget: int) -> str:
        """Ask the configured provider for a compact summary, with heuristic fallback."""
        trimmed_source = self._trim_text_to_budget(text, max(token_budget * 3, token_budget))
        if not trimmed_source:
            return ""
        if self.estimate_tokens(trimmed_source) <= token_budget:
                    ...
                    messages=[
                        {
                            ...
                            % (purpose, token_budget, trimmed_source),
                        }
                    ],
                    tool_schemas=[],
                )
        ...
        bullets: List[str] = []
        for raw_line in trimmed_source.splitlines():
            line = raw_line.strip()
            if not line:
                continue
        ...
        return self._trim_text_to_budget(rendered or shorten(trimmed_source, token_budget * 4), token_budget)
    ...
    def _find_tail_cut_by_tokens(self, messages: Sequence[Dict[str, object]], head_end: int, *, aggressive: bool = False) -> int:
        """Find the tail boundary by token budget rather than a fixed count."""
        token_budget = self._tail_token_budget(aggressive=aggressive)
        min_tail = min(
            len(messages) - head_end,
            max(1, int(self.config.context.compression_min_recent_messages)),
        )
        accumulated = 0
        ...
        for index in range(len(messages) - 1, head_end - 1, -1):
            message_tokens = self._message_token_cost(dict(messages[index]))
            if accumulated + message_tokens > soft_ceiling and (len(messages) - index) >= min_tail:
                break
            accumulated += message_tokens
            cut_index = index

        fallback_cut = max(head_end + 1, len(messages) - min_tail)
        if cut_index > fallback_cut:
            cut_index = fallback_cut
        if cut_index <= head_end:
            cut_index = fallback_cut
        cut_index = min(len(messages), cut_index)
        return self._align_boundary_backward(messages, cut_index)
    ...
    def compact_provider_messages(
        ...
        *,
        ...
        min_recent = max(1, int(self.config.context.compression_min_recent_messages))
        ...
        if not older_messages:
            ...
            while (
                ...
                > threshold
                and len(raw_recent) > min_recent
            ):
                raw_recent = raw_recent[1:]
    ...
    def build_provider_messages(
        ...
        *,
        ...
        history_messages = [{"id": item.get("id"), "role": item["role"], "content": str(item["content"])} for item in history]
    ...
    def _render_provider_round(self, item: Dict[str, object]) -> str:
        ...
        if response.get("content"):
            lines.append("Response Text: %s" % response.get("content", ""))
    ...
    def _render_conversation_event(self, item: Dict[str, object]) -> str:
        ...
        return "[%s] %s" % (label, item.get("content", ""))
