...
## Execution
- Let ordinary turns carry the reasoning; in research mode, project memory is archived after each turn.
...
- Preserve project memory through the research-log archival pass; do not use active-save tools as the main memory mechanism.
- Keep the current problem in `workspace/problem.md`; use `memory/research_log.md` and `memory/research_log.jsonl` for the human-readable and machine-readable research record.
- Keep failed paths, examples, counterexamples, checks, plans, and branch notes in the research log rather than scattered workspace files.
