...
class MoonshineCLI(object):
    ...
    def process_input(self, line: str, state, *, auto_run_research: bool = True, max_iterations: int = 100) -> str:
        ...
        stripped = line.strip()
        if stripped == "/auto-run":
            return "Usage: /auto-run <prompt>"
        if stripped.startswith("/auto-run "):
            prompt = stripped.split(None, 1)[1].strip()
            if not prompt:
                return "Usage: /auto-run <prompt>"
            if state.mode != "research":
                return "/auto-run is only available in research mode."
            self._resolve_research_project_interactively(prompt, state)
            render_agent_events(
